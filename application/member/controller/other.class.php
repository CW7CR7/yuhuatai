...
class other{
	...
	public function wx_login(){
		...
		$appkey = get_config('wx_secret');
		$callback = U('member/other/wx_login'); //回调地址 默认此方法，不能更改
		
		yzm_base::load_common('wechat_auth.class.php', 'member');
		if($appid=='' || $appkey=='') showmsg('请配置微信AppID和AppSecret！', 'stop');		
		$wx_obj = new wechat_auth($appid, $appkey, $callback);

		if(!isset($_GET['code'])){
			set_cookie('_url', $_GET['url']);
			$wx_obj->get_authorize_url();
		}else{
			$code = $_GET['code'];
			$_SESSION['fromlogin'] = 'wx';
			
			$res = $wx_obj->get_access_token($_GET['state'], $code);
			if(!$res)  showmsg('非法请求！', 'stop');
			$userinfo = $wx_obj->get_user_info($res['access_token'], $res['openid']);
	
			$openid = $_SESSION['openid'] = $userinfo['openid'];

			if(!empty($openid)){


				$member = D('member');
				$where = array('username'=>$openid);
				...
				$redirect = get_cookie('_url');
				header("Location:$redirect");

				// $userid = D('member_authorization')->field('userid')->where(array('authname' => 'wx', 'token' => $openid ))->order('id DESC')->one();		
				// if($userid){
				// 	$this->_login($userid);	
				// }else{	
	...
	private function _checkLogin($userid){
